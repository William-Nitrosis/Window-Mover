...
positions = {
    "ctrl+alt+1": (-7, -30),
    "ctrl+alt+2": (-7, 35),
    "ctrl+alt+3": (100, 100),  # Example additional shortcut
}
...
def main():
    ...
    print("Hotkeys:")
    for hotkey, pos in positions.items():
        print(f"  - {hotkey}: Move the window to {pos}")
    ...
    for hotkey, pos in positions.items():
        keyboard.add_hotkey(hotkey, move_focused_window_to_position, args=pos)
